...
$plans_delete = NULL; // Initialize page object first

class cplans_delete extends cplans {
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("planslist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in plans class, plansinfo.php
...
// Create page object
if (!isset($plans_delete)) $plans_delete = new cplans_delete();

// Page init
$plans_delete->Page_Init();

// Page main
$plans_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$plans_delete->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var plans_delete = new ew_Page("plans_delete");
plans_delete.PageID = "delete"; // Page ID
var EW_PAGE_ID = plans_delete.PageID; // For backward compatibility

// Form object
var fplansdelete = new ew_Form("fplansdelete");

// Form_CustomValidate event
fplansdelete.Form_CustomValidate = 
...
fplansdelete.ValidateRequired = true;
<?php } else { ?>
fplansdelete.ValidateRequired = false; 
<?php } ?>
...
if ($plans_delete->Recordset = $plans_delete->LoadRecordset())
	$plans_deleteTotalRecs = $plans_delete->Recordset->RecordCount(); // Get record count
if ($plans_deleteTotalRecs <= 0) { // No record found, exit
	if ($plans_delete->Recordset)
		$plans_delete->Recordset->Close();
	$plans_delete->Page_Terminate("planslist.php"); // Return to list
}
?>
<div class="ewToolbar">
...
<?php $plans_delete->ShowPageHeader(); ?>
<?php
$plans_delete->ShowMessage();
?>
<form name="fplansdelete" id="fplansdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($plans_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $plans_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="plans">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($plans_delete->RecKeys as $key) { ?>
...
<?php if ($plans->name_th->Visible) { // name_th ?>
		<th><span id="elh_plans_name_th" class="plans_name_th"><?php echo $plans->name_th->FldCaption() ?></span></th>
<?php } ?>
<?php if ($plans->name_en->Visible) { // name_en ?>
		<th><span id="elh_plans_name_en" class="plans_name_en"><?php echo $plans->name_en->FldCaption() ?></span></th>
<?php } ?>
<?php if ($plans->image->Visible) { // image ?>
		<th><span id="elh_plans_image" class="plans_image"><?php echo $plans->image->FldCaption() ?></span></th>
<?php } ?>
<?php if ($plans->enable->Visible) { // enable ?>
		<th><span id="elh_plans_enable" class="plans_enable"><?php echo $plans->enable->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$plans_delete->RecCnt = 0;
$i = 0;
while (!$plans_delete->Recordset->EOF) {
	$plans_delete->RecCnt++;
	$plans_delete->RowCnt++;

	// Set row properties
	$plans->ResetAttrs();
	$plans->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$plans_delete->LoadRowValues($plans_delete->Recordset);

	// Render row
	$plans_delete->RenderRow();
?>
	<tr<?php echo $plans->RowAttributes() ?>>
<?php if ($plans->name_th->Visible) { // name_th ?>
		<td<?php echo $plans->name_th->CellAttributes() ?>>
<span id="el<?php echo $plans_delete->RowCnt ?>_plans_name_th" class="plans_name_th">
...
<span id="el<?php echo $plans_delete->RowCnt ?>_plans_name_en" class="plans_name_en">
...
<span id="el<?php echo $plans_delete->RowCnt ?>_plans_image" class="plans_image">
<span>
...
<span id="el<?php echo $plans_delete->RowCnt ?>_plans_enable" class="plans_enable">
...
<?php
	$plans_delete->Recordset->MoveNext();
}
$plans_delete->Recordset->Close();
?>
</tbody>
