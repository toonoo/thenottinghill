...
$appointments_edit = NULL; // Initialize page object first

class cappointments_edit extends cappointments {
	...
	// Page object name
	var $PageObjName = 'appointments_edit';
	...
	function Page_Main() {
		...
		if ($this->id->CurrentValue == "")
			$this->Page_Terminate("appointmentslist.php"); // Invalid key, return to list
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// date
			$this->date->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->date->CurrentValue, 9), NULL, $this->date->ReadOnly);
...
// Create page object
if (!isset($appointments_edit)) $appointments_edit = new cappointments_edit();

// Page init
$appointments_edit->Page_Init();

// Page main
$appointments_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$appointments_edit->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var appointments_edit = new ew_Page("appointments_edit");
appointments_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = appointments_edit.PageID; // For backward compatibility

// Form object
var fappointmentsedit = new ew_Form("fappointmentsedit");

// Validate form
fappointmentsedit.Validate = function() {
...
fappointmentsedit.Form_CustomValidate = 
...
fappointmentsedit.ValidateRequired = true;
<?php } else { ?>
fappointmentsedit.ValidateRequired = false; 
<?php } ?>
...
<?php $appointments_edit->ShowPageHeader(); ?>
<?php
$appointments_edit->ShowMessage();
?>
<form name="fappointmentsedit" id="fappointmentsedit" class="form-horizontal ewForm ewEditForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($appointments_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $appointments_edit->Token ?>">
<?php } ?>
...
<?php if ($appointments->id->Visible) { // id ?>
	<div id="r_id" class="form-group">
		<label id="elh_appointments_id" class="col-sm-2 control-label ewLabel"><?php echo $appointments->id->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $appointments->id->CellAttributes() ?>>
<span id="el_appointments_id">
<span<?php echo $appointments->id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $appointments->id->EditValue ?></p></span>
</span>
<input type="hidden" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($appointments->id->CurrentValue) ?>">
<?php echo $appointments->id->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<script type="text/javascript">
fappointmentsedit.Init();
</script>
<?php
$appointments_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$appointments_edit->Page_Terminate();
?>
