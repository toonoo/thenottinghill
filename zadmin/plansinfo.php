...
//
// Table class for plans
//
class cplans extends cTable {
	var $id;
	var $name_th;
	var $name_en;
	var $detail_th;
	...
	function __construct() {
		...
		// id
		$this->id = new cField('plans', 'plans', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// name_th
		$this->name_th = new cField('plans', 'plans', 'x_name_th', 'name_th', '`name_th`', '`name_th`', 200, -1, FALSE, '`name_th`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['name_th'] = &$this->name_th;

		// name_en
		$this->name_en = new cField('plans', 'plans', 'x_name_en', 'name_en', '`name_en`', '`name_en`', 200, -1, FALSE, '`name_en`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['name_en'] = &$this->name_en;

		// detail_th
		$this->detail_th = new cField('plans', 'plans', 'x_detail_th', 'detail_th', '`detail_th`', '`detail_th`', 201, -1, FALSE, '`detail_th`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['detail_th'] = &$this->detail_th;

		// detail_en
		$this->detail_en = new cField('plans', 'plans', 'x_detail_en', 'detail_en', '`detail_en`', '`detail_en`', 201, -1, FALSE, '`detail_en`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['detail_en'] = &$this->detail_en;

		// image
		$this->image = new cField('plans', 'plans', 'x_image', 'image', '`image`', '`image`', 200, -1, TRUE, '`image`', FALSE, FALSE, FALSE, 'IMAGE');
		$this->fields['image'] = &$this->image;

		// enable
		$this->enable = new cField('plans', 'plans', 'x_enable', 'enable', '`enable`', '`enable`', 202, -1, FALSE, '`enable`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->enable->FldDataType = EW_DATATYPE_BOOLEAN;
		$this->fields['enable'] = &$this->enable;

		// created_date
		$this->created_date = new cField('plans', 'plans', 'x_created_date', 'created_date', '`created_date`', 'DATE_FORMAT(`created_date`, \'%Y/%m/%d %H:%i:%s\')', 135, 9, FALSE, '`created_date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		...
		// modified_date
		$this->modified_date = new cField('plans', 'plans', 'x_modified_date', 'modified_date', '`modified_date`', 'DATE_FORMAT(`modified_date`, \'%Y/%m/%d %H:%i:%s\')', 135, 9, FALSE, '`modified_date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function GetDetailUrl() {
		...
		// Detail url
		$sDetailUrl = "";
		if ($this->getCurrentDetailTable() == "plans_house") {
			$sDetailUrl = $GLOBALS["plans_house"]->GetListUrl() . "?showmaster=" . $this->TableVar;
			$sDetailUrl .= "&fk_id=" . urlencode($this->id->CurrentValue);
		}
		if ($sDetailUrl == "") {
			$sDetailUrl = "planslist.php";
		}
		return $sDetailUrl;
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`plans`";
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("plansview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("plansview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "plansadd.php?" . $this->UrlParm($parm);
		else
			return "plansadd.php";
	}
	...
	function GetEditUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("plansedit.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("plansedit.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
	...
	function GetCopyUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("plansadd.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("plansadd.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("plansdelete.php", $this->UrlParm());
	}
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->detail_th->Exportable) $Doc->ExportCaption($this->detail_th);
					if ($this->detail_en->Exportable) $Doc->ExportCaption($this->detail_en);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->detail_th->Exportable) $Doc->ExportField($this->detail_th);
						if ($this->detail_en->Exportable) $Doc->ExportField($this->detail_en);
