...
$plans_house_grid = NULL; // Initialize page object first

class cplans_house_grid extends cplans_house {
	...
	// Page object name
	var $PageObjName = 'plans_house_grid';

	// Grid form hidden field names
	var $FormName = 'fplans_housegrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["plans_house"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_name_th") && $objForm->HasValue("o_name_th") && $this->name_th->CurrentValue <> $this->name_th->OldValue)
			return FALSE;
		...
		if (!ew_Empty($this->image->Upload->Value))
			return FALSE;
		if ($objForm->HasValue("x_enable") && $objForm->HasValue("o_enable") && ew_ConvertToBool($this->enable->CurrentValue) <> ew_ConvertToBool($this->enable->OldValue))
			return FALSE;
		return TRUE;
	}
	...
	function LoadFormValues() {
		...
		$this->name_th->setOldValue($objForm->GetValue("o_name_th"));
		...
		$this->enable->setOldValue($objForm->GetValue("o_enable"));
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "plans") {
			$this->plans_id->Visible = FALSE;
			if ($GLOBALS["plans"]->EventCancelled) $this->EventCancelled = TRUE;
		}
