...
class cnews_gallery_edit extends cnews_gallery {
	...
	function SetupBreadcrumb() {
		...
		$Breadcrumb->Add("list", $this->TableVar, "news_gallerylist.php", "", $this->TableVar, TRUE);
		$PageId = "edit";
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_CheckDate(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($news_gallery->created_date->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_modified_date");
			if (elm && !ew_CheckDate(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($news_gallery->modified_date->FldErrMsg()) ?>");
...
?>
<form name="fnews_galleryedit" id="fnews_galleryedit" class="form-horizontal ewForm ewEditForm" action="<?php echo ew_CurrentPage() ?>" method="post">
...
<div>
<?php if ($news_gallery->id->Visible) { // id ?>
	<div id="r_id" class="form-group">
		<label id="elh_news_gallery_id" class="col-sm-2 control-label ewLabel"><?php echo $news_gallery->id->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $news_gallery->id->CellAttributes() ?>>
<span id="el_news_gallery_id">
<span<?php echo $news_gallery->id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $news_gallery->id->EditValue ?></p></span>
</span>
<input type="hidden" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($news_gallery->id->CurrentValue) ?>">
<?php echo $news_gallery->id->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($news_gallery->image->Visible) { // image ?>
	<div id="r_image" class="form-group">
		<label id="elh_news_gallery_image" for="x_image" class="col-sm-2 control-label ewLabel"><?php echo $news_gallery->image->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $news_gallery->image->CellAttributes() ?>>
<span id="el_news_gallery_image">
<input type="text" data-field="x_image" name="x_image" id="x_image" size="30" maxlength="255" placeholder="<?php echo ew_HtmlEncode($news_gallery->image->PlaceHolder) ?>" value="<?php echo $news_gallery->image->EditValue ?>"<?php echo $news_gallery->image->EditAttributes() ?>>
</span>
<?php echo $news_gallery->image->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($news_gallery->enable->Visible) { // enable ?>
	<div id="r_enable" class="form-group">
		<label id="elh_news_gallery_enable" class="col-sm-2 control-label ewLabel"><?php echo $news_gallery->enable->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $news_gallery->enable->CellAttributes() ?>>
<span id="el_news_gallery_enable">
<div id="tp_x_enable" class="<?php echo EW_ITEM_TEMPLATE_CLASSNAME ?>"><input type="radio" name="x_enable" id="x_enable" value="{value}"<?php echo $news_gallery->enable->EditAttributes() ?>></div>
<div id="dsl_x_enable" data-repeatcolumn="5" class="ewItemList">
<?php
$arwrk = $news_gallery->enable->EditValue;
if (is_array($arwrk)) {
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($news_gallery->enable->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " checked=\"checked\"" : "";
		if ($selwrk <> "") $emptywrk = FALSE;

		// Note: No spacing within the LABEL tag
?>
<?php echo ew_RepeatColumnTable($rowswrk, $rowcntwrk, 5, 1) ?>
<label class="radio-inline"><input type="radio" data-field="x_enable" name="x_enable" id="x_enable_<?php echo $rowcntwrk ?>" value="<?php echo ew_HtmlEncode($arwrk[$rowcntwrk][0]) ?>"<?php echo $selwrk ?><?php echo $news_gallery->enable->EditAttributes() ?>><?php echo $arwrk[$rowcntwrk][1] ?></label>
...
<?php echo $news_gallery->enable->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($news_gallery->created_date->Visible) { // created_date ?>
	<div id="r_created_date" class="form-group">
		<label id="elh_news_gallery_created_date" for="x_created_date" class="col-sm-2 control-label ewLabel"><?php echo $news_gallery->created_date->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $news_gallery->created_date->CellAttributes() ?>>
<span id="el_news_gallery_created_date">
<input type="text" data-field="x_created_date" name="x_created_date" id="x_created_date" placeholder="<?php echo ew_HtmlEncode($news_gallery->created_date->PlaceHolder) ?>" value="<?php echo $news_gallery->created_date->EditValue ?>"<?php echo $news_gallery->created_date->EditAttributes() ?>>
</span>
<?php echo $news_gallery->created_date->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($news_gallery->modified_date->Visible) { // modified_date ?>
	<div id="r_modified_date" class="form-group">
		<label id="elh_news_gallery_modified_date" for="x_modified_date" class="col-sm-2 control-label ewLabel"><?php echo $news_gallery->modified_date->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $news_gallery->modified_date->CellAttributes() ?>>
<span id="el_news_gallery_modified_date">
<input type="text" data-field="x_modified_date" name="x_modified_date" id="x_modified_date" placeholder="<?php echo ew_HtmlEncode($news_gallery->modified_date->PlaceHolder) ?>" value="<?php echo $news_gallery->modified_date->EditValue ?>"<?php echo $news_gallery->modified_date->EditAttributes() ?>>
</span>
<?php echo $news_gallery->modified_date->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div>
