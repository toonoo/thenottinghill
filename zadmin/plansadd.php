...
$plans_add = NULL; // Initialize page object first

class cplans_add extends cplans {
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "plansview.php")
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			if ($this->image->UseColorbox) {
				...
				$this->image->LinkAttrs["data-rel"] = "plans_x_image";
	...
	function ValidateForm() {
		...
		if (in_array("plans_house", $DetailTblVar) && $GLOBALS["plans_house"]->DetailAdd) {
			if (!isset($GLOBALS["plans_house_grid"])) $GLOBALS["plans_house_grid"] = new cplans_house_grid(); // get detail page object
			$GLOBALS["plans_house_grid"]->ValidateGridForm();
		}
	...
	function AddRow($rsold = NULL) {
		...
		if ($AddRow) {
			$DetailTblVar = explode(",", $this->getCurrentDetailTable());
			if (in_array("plans_house", $DetailTblVar) && $GLOBALS["plans_house"]->DetailAdd) {
				$GLOBALS["plans_house"]->plans_id->setSessionValue($this->id->CurrentValue); // Set master key
				if (!isset($GLOBALS["plans_house_grid"])) $GLOBALS["plans_house_grid"] = new cplans_house_grid(); // Get detail page object
				$AddRow = $GLOBALS["plans_house_grid"]->GridInsert();
				if (!$AddRow)
					$GLOBALS["plans_house"]->plans_id->setSessionValue(""); // Clear master key if insert failed
			}
		}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("plans_house", $DetailTblVar)) {
				if (!isset($GLOBALS["plans_house_grid"]))
					$GLOBALS["plans_house_grid"] = new cplans_house_grid;
				if ($GLOBALS["plans_house_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["plans_house_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["plans_house_grid"]->CurrentMode = "add";
					$GLOBALS["plans_house_grid"]->CurrentAction = "gridadd";

					// Save current master table to detail table
					$GLOBALS["plans_house_grid"]->setCurrentMasterTable($this->TableVar);
					$GLOBALS["plans_house_grid"]->setStartRecordNumber(1);
					$GLOBALS["plans_house_grid"]->plans_id->FldIsDetailKey = TRUE;
					$GLOBALS["plans_house_grid"]->plans_id->CurrentValue = $this->id->CurrentValue;
					$GLOBALS["plans_house_grid"]->plans_id->setSessionValue($GLOBALS["plans_house_grid"]->plans_id->CurrentValue);
				}
			}
		}
	}
	...
	function SetupBreadcrumb() {
		...
		$Breadcrumb->Add("list", $this->TableVar, "planslist.php", "", $this->TableVar, TRUE);
...
// Create page object
if (!isset($plans_add)) $plans_add = new cplans_add();
...
$plans_add->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var plans_add = new ew_Page("plans_add");
plans_add.PageID = "add"; // Page ID
var EW_PAGE_ID = plans_add.PageID; // For backward compatibility

// Form object
var fplansadd = new ew_Form("fplansadd");

// Validate form
fplansadd.Validate = function() {
...
fplansadd.ValidateRequired = true;
<?php } else { ?>
fplansadd.ValidateRequired = false; 
<?php } ?>
...
<?php $plans_add->ShowPageHeader(); ?>
<?php
$plans_add->ShowMessage();
?>
<form name="fplansadd" id="fplansadd" class="form-horizontal ewForm ewAddForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($plans_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $plans_add->Token ?>">
<?php } ?>
...
<?php if ($plans->name_th->Visible) { // name_th ?>
	<div id="r_name_th" class="form-group">
		<label id="elh_plans_name_th" for="x_name_th" class="col-sm-2 control-label ewLabel"><?php echo $plans->name_th->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $plans->name_th->CellAttributes() ?>>
<span id="el_plans_name_th">
<input type="text" data-field="x_name_th" name="x_name_th" id="x_name_th" size="50" maxlength="255" placeholder="<?php echo ew_HtmlEncode($plans->name_th->PlaceHolder) ?>" value="<?php echo $plans->name_th->EditValue ?>"<?php echo $plans->name_th->EditAttributes() ?>>
</span>
<?php echo $plans->name_th->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($plans->name_en->Visible) { // name_en ?>
	<div id="r_name_en" class="form-group">
		<label id="elh_plans_name_en" for="x_name_en" class="col-sm-2 control-label ewLabel"><?php echo $plans->name_en->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $plans->name_en->CellAttributes() ?>>
<span id="el_plans_name_en">
<input type="text" data-field="x_name_en" name="x_name_en" id="x_name_en" size="50" maxlength="255" placeholder="<?php echo ew_HtmlEncode($plans->name_en->PlaceHolder) ?>" value="<?php echo $plans->name_en->EditValue ?>"<?php echo $plans->name_en->EditAttributes() ?>>
</span>
<?php echo $plans->name_en->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($plans->detail_th->Visible) { // detail_th ?>
	<div id="r_detail_th" class="form-group">
		<label id="elh_plans_detail_th" class="col-sm-2 control-label ewLabel"><?php echo $plans->detail_th->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $plans->detail_th->CellAttributes() ?>>
<span id="el_plans_detail_th">
<?php ew_AppendClass($plans->detail_th->EditAttrs["class"], "editor"); ?>
<textarea data-field="x_detail_th" name="x_detail_th" id="x_detail_th" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($plans->detail_th->PlaceHolder) ?>"<?php echo $plans->detail_th->EditAttributes() ?>><?php echo $plans->detail_th->EditValue ?></textarea>
<script type="text/javascript">
ew_CreateEditor("fplansadd", "x_detail_th", 35, 4, <?php echo ($plans->detail_th->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
<?php echo $plans->detail_th->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($plans->detail_en->Visible) { // detail_en ?>
	<div id="r_detail_en" class="form-group">
		<label id="elh_plans_detail_en" class="col-sm-2 control-label ewLabel"><?php echo $plans->detail_en->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $plans->detail_en->CellAttributes() ?>>
<span id="el_plans_detail_en">
<?php ew_AppendClass($plans->detail_en->EditAttrs["class"], "editor"); ?>
<textarea data-field="x_detail_en" name="x_detail_en" id="x_detail_en" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($plans->detail_en->PlaceHolder) ?>"<?php echo $plans->detail_en->EditAttributes() ?>><?php echo $plans->detail_en->EditValue ?></textarea>
<script type="text/javascript">
ew_CreateEditor("fplansadd", "x_detail_en", 35, 4, <?php echo ($plans->detail_en->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
<?php echo $plans->detail_en->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($plans->image->Visible) { // image ?>
	<div id="r_image" class="form-group">
		<label id="elh_plans_image" class="col-sm-2 control-label ewLabel"><?php echo $plans->image->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $plans->image->CellAttributes() ?>>
<span id="el_plans_image">
<div id="fd_x_image">
<span title="<?php echo $plans->image->FldTitle() ? $plans->image->FldTitle() : $Language->Phrase("ChooseFile") ?>" class="btn btn-default btn-sm fileinput-button ewTooltip<?php if ($plans->image->ReadOnly || $plans->image->Disabled) echo " hide"; ?>">
	<span><?php echo $Language->Phrase("ChooseFileBtn") ?></span>
	<input type="file" title=" " data-field="x_image" name="x_image" id="x_image">
</span>
<input type="hidden" name="fn_x_image" id= "fn_x_image" value="<?php echo $plans->image->Upload->FileName ?>">
<input type="hidden" name="fa_x_image" id= "fa_x_image" value="0">
<input type="hidden" name="fs_x_image" id= "fs_x_image" value="255">
<input type="hidden" name="fx_x_image" id= "fx_x_image" value="<?php echo $plans->image->UploadAllowedFileExt ?>">
<input type="hidden" name="fm_x_image" id= "fm_x_image" value="<?php echo $plans->image->UploadMaxFileSize ?>">
</div>
<table id="ft_x_image" class="table table-condensed pull-left ewUploadTable"><tbody class="files"></tbody></table>
</span>
<?php echo $plans->image->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div>
<?php
	if (in_array("plans_house", explode(",", $plans->getCurrentDetailTable())) && $plans_house->DetailAdd) {
?>
<?php if ($plans->getCurrentDetailTable() <> "") { ?>
<h4 class="ewDetailCaption"><?php echo $Language->TablePhrase("plans_house", "TblCaption") ?></h4>
<?php } ?>
<?php include_once "plans_housegrid.php" ?>
<?php } ?>
