...
$progress_view = NULL; // Initialize page object first

class cprogress_view extends cprogress {
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["id"] <> "") {
				...
				$sReturnUrl = "progresslist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "progresslist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "progresslist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($progress_view)) $progress_view = new cprogress_view();

// Page init
$progress_view->Page_Init();

// Page main
$progress_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$progress_view->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var progress_view = new ew_Page("progress_view");
progress_view.PageID = "view"; // Page ID
var EW_PAGE_ID = progress_view.PageID; // For backward compatibility

// Form object
var fprogressview = new ew_Form("fprogressview");

// Form_CustomValidate event
fprogressview.Form_CustomValidate = 
...
fprogressview.ValidateRequired = true;
<?php } else { ?>
fprogressview.ValidateRequired = false; 
<?php } ?>
...
<?php $progress_view->ExportOptions->Render("body") ?>
<?php
	foreach ($progress_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php echo $Language->SelectionForm(); ?>
<div class="clearfix"></div>
</div>
<?php $progress_view->ShowPageHeader(); ?>
<?php
$progress_view->ShowMessage();
?>
<form name="fprogressview" id="fprogressview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($progress_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $progress_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="progress">
<table class="table table-bordered table-striped ewViewTable">
<?php if ($progress->image->Visible) { // image ?>
	<tr id="r_image">
		<td><span id="elh_progress_image"><?php echo $progress->image->FldCaption() ?></span></td>
		<td<?php echo $progress->image->CellAttributes() ?>>
<span id="el_progress_image">
<span>
<?php echo ew_GetFileViewTag($progress->image, $progress->image->ViewValue) ?>
</span>
</span>
</td>
	</tr>
...
<?php if ($progress->project_date->Visible) { // project_date ?>
	<tr id="r_project_date">
		<td><span id="elh_progress_project_date"><?php echo $progress->project_date->FldCaption() ?></span></td>
...
<?php echo $progress->project_date->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($progress->enable->Visible) { // enable ?>
	<tr id="r_enable">
		<td><span id="elh_progress_enable"><?php echo $progress->enable->FldCaption() ?></span></td>
...
<input type="checkbox" value="<?php echo $progress->enable->ViewValue ?>" checked="checked" disabled="disabled">
<?php } else { ?>
<input type="checkbox" value="<?php echo $progress->enable->ViewValue ?>" disabled="disabled">
<?php } ?>
</span>
</span>
</td>
...
<?php if ($progress->created_date->Visible) { // created_date ?>
	<tr id="r_created_date">
		<td><span id="elh_progress_created_date"><?php echo $progress->created_date->FldCaption() ?></span></td>
		<td<?php echo $progress->created_date->CellAttributes() ?>>
<span id="el_progress_created_date">
<span<?php echo $progress->created_date->ViewAttributes() ?>>
<?php echo $progress->created_date->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($progress->modified_date->Visible) { // modified_date ?>
	<tr id="r_modified_date">
		<td><span id="elh_progress_modified_date"><?php echo $progress->modified_date->FldCaption() ?></span></td>
		<td<?php echo $progress->modified_date->CellAttributes() ?>>
<span id="el_progress_modified_date">
<span<?php echo $progress->modified_date->ViewAttributes() ?>>
<?php echo $progress->modified_date->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
