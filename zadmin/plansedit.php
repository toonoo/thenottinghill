...
class cplans_edit extends cplans {
	...
	function ValidateForm() {
		...
		if (in_array("plans_house", $DetailTblVar) && $GLOBALS["plans_house"]->DetailEdit) {
		...
		if ($rs->EOF) {
			...
			if ($bUpdateRow) {
				...
				if ($EditRow) {
					...
					if (in_array("plans_house", $DetailTblVar) && $GLOBALS["plans_house"]->DetailEdit) {
						if (!isset($GLOBALS["plans_house_grid"])) $GLOBALS["plans_house_grid"] = new cplans_house_grid(); // Get detail page object
						$EditRow = $GLOBALS["plans_house_grid"]->GridUpdate();
					}
				}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("plans_house", $DetailTblVar)) {
				...
				if ($GLOBALS["plans_house_grid"]->DetailEdit) {
					$GLOBALS["plans_house_grid"]->CurrentMode = "edit";
					$GLOBALS["plans_house_grid"]->CurrentAction = "gridedit";
...
// Page object
var plans_edit = new ew_Page("plans_edit");
plans_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = plans_edit.PageID; // For backward compatibility

// Form object
var fplansedit = new ew_Form("fplansedit");
...
?>
<form name="fplansedit" id="fplansedit" class="form-horizontal ewForm ewEditForm" action="<?php echo ew_CurrentPage() ?>" method="post">
...
<script type="text/javascript">
ew_CreateEditor("fplansedit", "x_detail_th", 35, 4, <?php echo ($plans->detail_th->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
...
<script type="text/javascript">
ew_CreateEditor("fplansedit", "x_detail_en", 35, 4, <?php echo ($plans->detail_en->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
...
<input type="hidden" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($plans->id->CurrentValue) ?>">
<?php
	if (in_array("plans_house", explode(",", $plans->getCurrentDetailTable())) && $plans_house->DetailEdit) {
?>
